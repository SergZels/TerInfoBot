...
def getstring(li :list)->str:
    ...
    if li['email']!="":
        str += f"<strong>e-mail:</strong> {li['email']}\n "
        ...
        try:
           ...
           # res = f"Назва: {i['Name']}\nОпис: {i['About']}\nАдреса: {i['address']}\nТел.: {i['tel']}\nГрафік: {i['work_schedule']}\nСайт:{i['SiteURL']}\n{listindex+1} із {len(main_list)}\n"
            ...
            await bot.edit_message_media(chat_id=query.message.chat.id, message_id=query.message.message_id,
                                     ...
                                     reply_markup=keyboard_prev_next_about)
...
async def change_image_callback(query: types.CallbackQuery, state: FSMContext):
    ...
    i = main_list[listindex]
    res= f"{getstring(i)}\n1 із {len(main_list)}\n"
    await bot.edit_message_media(chat_id=query.message.chat.id, message_id=query.message.message_id,media=types.InputMediaPhoto(media=i['PhotoURL'],caption=" "))
...
async def change_image_callback(query: types.CallbackQuery, state: FSMContext):
    ...
    if len(resp.text) > 2:
        ...
        #res= f"{getstring(i)}\n1 із {len(main_list)}\n"
        try:
         await bot.send_photo(chat_id=query.message.chat.id, photo=i['PhotoURL'], caption=res, reply_markup=keyboard_prev_next_about)
        # await bot.send_message(chat_id=query.message.chat.id,text=res, reply_markup=keyboard_prev_next_about, parse_mode="HTML")
        except:
